...
function showResults(predictionData) {
    ...
    // Add detailed logging to help identify matching issues
    console.log("API returned condition:", predictedClass);
    console.log("Available conditions in database:", skinConditions.map(c => c.name));
    
    // Improved matching approach with more robust string comparison
    let conditionDetails = null;
    
    // First try exact match
    conditionDetails = skinConditions.find(
        condition => condition.name === predictedClass
    );
    ...
    if (!conditionDetails) {
        conditionDetails = skinConditions.find(
            condition => condition.name.toLowerCase().includes(predictedClass.toLowerCase()) || 
    ...
    if (conditionDetails) {
        ...
        console.log(`Match found! Displaying details for "${conditionDetails.name}" with ${confidencePercentage}% confidence.`);
        ...
        const confidencePercentage = (apiConfidence * 100).toFixed(2);
        console.warn(`Predicted class "${predictedClass}" not found in local skinConditions database. Check spelling and formatting.`);
        diagnosisHTML = `
             <div class="diagnosis-name">
